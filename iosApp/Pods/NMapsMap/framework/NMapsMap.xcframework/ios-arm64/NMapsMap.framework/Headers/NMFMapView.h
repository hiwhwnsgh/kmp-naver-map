...
@protocol NMFAttributionChangeDelegate;
...
/**
 화면에 보이는 지도 데이터의 출처. 단, 네이버는 출처에서 생략됩니다. 즉, 이 메서드의 반환 배열에
 네이버에 대한 정보가 없더라도, 지도 데이터의 출처가 네이버일 수 있습니다.
 */
@property (nonatomic, nonnull, readonly) NSArray<NMFAttribution *> *attributions;

/**
 지도 데이터 출처 변경에 위임자를 등록합니다.
 
 @param delegate `NMFAttributionChangeDelegate` 객체.
 */
- (void)addAttributionChangeDelegate:(id<NMFAttributionChangeDelegate> _Nonnull)delegate NS_SWIFT_NAME(addAttributionChangeDelegate(delegate:));

/**
 지도 데이터 출처 변경에 위임자를 해제합니다.
 
 @param delegate `NMFAttributionChangeDelegate` 객체.
 */
- (void)removeAttributionChangeDelegate:(id<NMFAttributionChangeDelegate> _Nonnull)delegate NS_SWIFT_NAME(removeAttributionChangeDelegate(delegate:));
 ...
 */
@property (nonatomic, nullable) NSString *customStyleId;
 ...
 */
- (void)setCustomStyleId:(NSString * _Nullable)customStyleId
